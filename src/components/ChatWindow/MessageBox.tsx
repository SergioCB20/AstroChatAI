...
export default function MessageBox({ message }: { message: Message }) {
  ...
  return (
    <div className={`w-fit max-w-[80%] relative flex gap-2 h-fit p-5 mb-5 border-2 border-black ${message.user.id === user?.id ? 'self-end' : 'self-start'}`}>
      <p className='w-fit max-w-[100%] break-words text-sm lg:pe-12 pe-5'>{message.text}</p>
      <div>
